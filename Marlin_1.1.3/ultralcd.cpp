...
//SoftwareSerial HMI_SERIAL(17, 16); //UART2 Tx:16, Rx:17 /// UART1 Tx:18, Rx:19 /// UART3 Tx:14, Rx:15
...
void lcd_init() {
  ...
  #if ENABLED(SDSUPPORT) && PIN_EXISTS(SD_DETECT_PIN)
...
#define fanSpeed fanSpeeds[0]
#define feedmultiply feedrate_percentage
//#define target_temperature_bed thermalManager.target_temperature_bed
#define axis_steps_per_unit planner.axis_steps_per_mm
//#define current_temperature thermalManager.degHotend(active_extruder)
#define plan_buffer_line planner.buffer_line
///////////////////////////////////////////////

float degHotend(int nr)
{
  return thermalManager.degHotend(nr);
}

float current_temperature(int nr)
{
  return thermalManager.degHotend(nr);
}
...
void setTargetHotend0(float h_target)
{
  thermalManager.setTargetHotend(h_target, 0);
}

void setTargetHotend(float h_target, int active_ex)
{
  thermalManager.setTargetHotend(h_target, active_ex);
}

void setTargetBed(float b_target)
{
  thermalManager.setTargetBed(b_target);
}
...
FORCE_INLINE void autotempShutdown(){
 ...
 if(planner.autotemp_enabled)
 {
  ...
  if(thermalManager.degHotend(active_extruder)>planner.autotemp_min)
...
void lcd_vs_App_Page(char temp_page)  // Request LCD load a display page
{
  for(int i =0; i <2;i++){  // Send page request twice... (why!?)
    vs_lcd_data[0]=0x5A;
    vs_lcd_data[1]=0xA5;
    vs_lcd_data[2]=0x04;
    vs_lcd_data[3]=0x80;
    ...
    {
      HMI_SERIAL.write(vs_lcd_data[i]);
      //MYSERIAL.write(vs_lcd_data[i]);
      _delay_ms(2);
    }
  }
}

void lcd_vs_temp_data()  // send temperature data to LCD
{
  ...
  tmp_extru=thermalManager.degHotend(active_extruder);//thermalManager.target_temperature[0];//degHotend(tmp1_extruder);
  if(tmp_extru>255)
  {
    vs_lcd_data[6]=0x01;
  ...
  vs_lcd_data[9]=0x00;
  vs_lcd_data[10]=0x00;
  vs_lcd_data[11]=thermalManager.degBed();//TEMP_SENSOR_BED;//thermalManager.target_temperature_bed;// (char)((int)degBed());
  vs_lcd_data[12]=0x00;
  vs_lcd_data[13]=fanSpeeds[0]*100/255;
  vs_lcd_data[14]=0x00;
  ...
  vs_lcd_data[25]=0x00;
    ...
    int tmp_extru;
    tmp_extru=thermalManager.target_temperature[0];
    if(tmp_extru>255)
    {
      vs_lcd_data[26]=0x01;
      tmp_extru=tmp_extru-256;
  ...
  vs_lcd_data[30]=0x00;
  vs_lcd_data[31]=thermalManager.target_temperature_bed;//(char)target_temperature_bed;

    int temp_print_percent=feedrate_percentage;//feedmultiply;
...
void lcd_vs_command()
{
  //MYSERIAL.println("Checking Commands");
  ...
  { 
    ...
    while (HMI_SERIAL.available()>0)
    {
      ...
      vs_temp_data = char(HMI_SERIAL.read());
    ...
    case 50 : 
      ...
      for(uint8_t i=0;i<4;i++)
      {
        ...
        for(int u=0; u<20 ;u++)
        {
          HMI_SERIAL.write(sdfilename[u]);
          _delay_ms(2);
        }
      }

      if(hmi_lang > 1)//(acceleration>1000)
    ...
    case 51 :
      {
        ...
        { 
          ...
          if( card.cardOK )
          {
            ...
            HMI_SERIAL.println(card.filename);
          ...
          for(int yi=0 ; yi<13 ;yi++)
          {
            sdfilename[yi+6]=card.longFilename[yi];   //��������ַ��͸�ֵ
          }
          for(int u=0; u<20 ;u++)
          {
            HMI_SERIAL.write(sdfilename[u]);
    ...
    //    lcd_vs_App_Page(0x41); //3.5
      ...
      //  _delay_ms(10);
        ...
        //��ת����ӡ����
        if(hmi_lang > 1)//(acceleration>1000)
          lcd_vs_App_Page(0x49);//����
        else
          lcd_vs_App_Page(0xA1);//Ӣ��
        break; 
    ...
    case 58:
      ...
      //  target_temperature_bed=(int)lcd_vs_com_data[8];
        if(lcd_vs_com_data[8]>0)
        {
          ...
          if(thermalManager.target_temperature_bed<125)
    ...
    case 62:
      {
        ...
        //PID - P
        vs_lcd_data[14]=0x00;
        vs_lcd_data[15]=PID_PARAM(Kp, 1);//Kp;
        //PID - I  DEFAULT_Ki
        vs_lcd_data[16]=0x00;
        vs_lcd_data[17]=unscalePID_i(PID_PARAM(Ki, 1)); //unscalePID_i(Ki);
        //PID - D DEFAULT_Kd
        vs_lcd_data[18]=0x00;
        vs_lcd_data[19]=unscalePID_d(PID_PARAM(Kd, 1)); //unscalePID_d(Kd);
        ...
        { //PID ҳ��
          ...
          //�������Ӣ�ģ�������ת
          if(hmi_lang > 1)//(acceleration>1000)
            lcd_vs_App_Page(0x54);//����
          else
            lcd_vs_App_Page(0xAC);//Ӣ��
        }
        else
        { //��� ҳ��
          ...
          //�������Ӣ�ģ�������ת
          if(hmi_lang > 1)//(acceleration>1000)
            lcd_vs_App_Page(0x56);//����
          else
            lcd_vs_App_Page(0xAE);//Ӣ��
        }
    ...
    case 63:
        {
          ...
          do
          {
            ...
            {
              //�����ֽڣ�ת��Ϊ�ַ���
              mychar[iu]=char(HMI_SERIAL.read());
              iu++;
              _delay_ms(2); //��ʱ2����
            }
          }while(1);
          ...
          //  axis_steps_per_unit[Z_AXIS]+=lcd_vs_com_data[12];
            ...
            //PID
            float temp_Kp;
            temp_Kp=mychar[15]*255+mychar[16];
            PID_PARAM(Kp, 1)=temp_Kp;
            PID_PARAM(Ki, 1)=scalePID_i(mychar[17]*255+mychar[18]); //Ki=scalePID_i(mychar[17]*255+mychar[18]);
            PID_PARAM(Kd, 1)=scalePID_d(mychar[19]*255+mychar[20]); //Kd=scalePID_d(mychar[19]*255+mychar[20]);
            thermalManager.updatePID();
          }
          ...
          //�������Ӣ�ģ�������ת
          if(hmi_lang > 1)//(acceleration>1000)
            lcd_vs_App_Page(0x52);//����
          else
            lcd_vs_App_Page(0xAA);//Ӣ��
    ...
    case 64:
      {
        ...
        //    _delay_ms(100);
          ...
          //    setE0temp(temp_target012);
            ...
            enqueue_and_echo_command(cmd1); //_enqueuecommand(cmd1, true); //enquecommand(cmd1); 
    ...
    case 65:
      { //��Ӣ�Ľ����л�
        if(hmi_lang > 1)//(acceleration>1000)  ///// NOTE: ACCELERATION WAS USED Because it gets saved into EEPROM!!  Therefore the language choice is 'saved'
          hmi_lang = 1; //acceleration=1000;
        else
          hmi_lang = 2; //acceleration=1002;
        //���ñ��浽EEPROM��
        Config_StoreSettings();
        break;
      }
    ...
    //    target_temperature_bed_tm=degBed();
      ...
      //  vs_lcd_data[9]=tmp_extru;
        vs_lcd_data[9]=(int)thermalManager.target_temperature[0];//(int)degTargetHotend(0);
        if(tmp_extru>255)
          vs_lcd_data[8]=(int)thermalManager.target_temperature[0] / 255; //(int)degTargetHotend(0)/255;
        else
          vs_lcd_data[8]=0x00;
        //�ȴ�Ŀ���¶�
        vs_lcd_data[10]=0x00;
        vs_lcd_data[11]=(int)thermalManager.target_temperature_bed;//(int)thermalManager.target_bed; //(int)degTargetBed();
      ...
      //  _delay_ms(10);
        if(hmi_lang > 1) //(acceleration>1000)
          lcd_vs_App_Page(0x4B);//����
        else
          lcd_vs_App_Page(0xA3);//Ӣ��
      ...
      case 72 : //�Զ�����
        {
          ...
          //  enquecommand_P((PSTR("G1 F100 E200"))); //F100 ת��  E200 ����200����

          //����
          setTargetHotend0(200);
          if(193<=current_temperature[0]) //�¶ȼ��
          {
            //  enquecommand_P((PSTR("G1 F25 ")));
            //����ͷ����4MM
            current_position[E_AXIS]+=0.68; //���϶��ٺ���
    ...
    case 73 : //�Զ�����
      {
        ...
        //  enquecommand_P((PSTR("G1 F10000 E-100"))); //F100 ת��  E200 ����200����
    ...
    case 74 : //�رռ���
      {
        ...
        //��ת����
        if(hmi_lang > 1) //(acceleration>1000)
          lcd_vs_App_Page(0xC3);//����
        else
          lcd_vs_App_Page(0xB2);//Ӣ��
        //break;
        //  enquecommand_P((PSTR("M109 S0"))); //���� 
    ...
    case 75 : //
      {
        ...
        {
          if(hmi_lang > 1) //(acceleration>1000)
            lcd_vs_App_Page(0xCA);//����
          else
            lcd_vs_App_Page(0xB9);//Ӣ��
          break;
        }
        _delay_ms(300);
        //���ͷ���ѭ��ָ��
    ...
    case 1:
      {
        ...
        //  enquecommand_P(PSTR("G1 X10")); // X��
        if(1==lcd_vs_com_data[8])
        {
          current_position[X_AXIS]-=10;
          plan_buffer_line(current_position[X_AXIS], current_position[Y_AXIS], current_position[Z_AXIS] , current_position[E_AXIS], manual_feedrate[X_AXIS]/60, active_extruder);
        }
        //  enquecommand_P(PSTR("G1 X-10")); // -X��
        ...
        //  enquecommand_P(PSTR("G1 Y10")); // Y��
        if(3==lcd_vs_com_data[8])
        {
          current_position[Y_AXIS]-=10;
          plan_buffer_line(current_position[X_AXIS], current_position[Y_AXIS], current_position[Z_AXIS] , current_position[E_AXIS], manual_feedrate[Y_AXIS]/60, active_extruder);
        }
        //  enquecommand_P(PSTR("G1 Y-10")); // -Y��
        ...
        //  enquecommand_P(PSTR("G1 Z5")); // Z��
        if(5==lcd_vs_com_data[8])
        {
          current_position[Z_AXIS]-=5;
          plan_buffer_line(current_position[X_AXIS], current_position[Y_AXIS], current_position[Z_AXIS] , current_position[E_AXIS], manual_feedrate[Z_AXIS]/60, active_extruder);
        }
        //  enquecommand_P(PSTR("G1 Z-5")); // -Z��
        ...
        //    enquecommand_P(PSTR("G1 E10")); // -E��
          if(7==lcd_vs_com_data[8])
          {
          current_position[E_AXIS]+=10;
          plan_buffer_line(current_position[X_AXIS], current_position[Y_AXIS], current_position[Z_AXIS] , current_position[E_AXIS], manual_feedrate[E_AXIS]/60, active_extruder);
          }
        //    enquecommand_P(PSTR("G1 E-10")); // E��
          //�رյ��E����
...
{
  lcd_vs_temp_data();  ////////////  UPDATE MonoPrice Temps
  oldtime=millis();
}
